...
import { 
  Upload, Image, FileText, Loader2, Download, 
  Trash2, Plus, X, Grid, List, Sparkles, CloudUpload, CheckCircle,
  ArrowLeft, Home, Layers, Zap, Shield, Clock, Cloud, AlertCircle
} from "lucide-react";
...
export default function ImageGenerator() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.log(
  "Full URL:",
  `${api.defaults.baseURL}/pdf/img`
);
      const res = await api.post("/pdf/img", formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (data.uploadedToDrive) {
        ...
        alert("PDF generated but Drive upload failed. Check your Google Drive connection.");
      } else {
        alert("PDF generated successfully!");
      }
    } catch (err) {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-0.5 mb-3">
                Connect your Google Drive to automatically save generated PDFs to the cloud.
              </p>
              <button
                onClick={handleGoogleConnect}
                className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-xs font-semibold rounded-lg hover:shadow-md hover:scale-[1.02] transition-all"
              >
                <Cloud size={13} />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="mt-10 grid grid-cols-1 md:grid-cols-4 gap-4">
          {[
            { icon: Zap, label: "Fast Conversion", sub: "Convert in seconds" },
            { icon: Shield, label: "High Quality", sub: "Preserve image quality" },
            { icon: CloudUpload, label: "Cloud Backup", sub: "Save to Google Drive" },
            { icon: Clock, label: "Batch Processing", sub: "Multiple images at once" },
