...
export default function FileCompressor() {
  ...
  const handleCompress = async () => {
    ...
    try {
      ...
      if (uploadToDrive) {
        alert(
          `${res.data.files.length} files uploaded to Google Drive!`
        );
      } else {
        alert(
          "Files compressed and downloaded successfully!"
        );
      }
    } catch (err) {
      // console.log(err);
  ...
  const stats = {
    totalFiles: files.length,
    totalSize: formatSize(totalSize),
    estimatedSize: formatSize(
      estimatedCompressedSize
    ),
    savings:
      totalSize > 0
        ? Math.round((1 - quality / 100) * 100)
        : 0,
  };
      ...
      return (
        <div className="mt-4 p-4 bg-gradient-to-r from-cyan-50 to-blue-50 rounded-xl border border-cyan-100">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 bg-cyan-100 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
              <Cloud
                size={16}
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-slate-500 mt-0.5 mb-3">
                ...
                automatically save compressed files
                to the cloud.
              </p>

              <button
                onClick={handleGoogleConnect}
                className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-xs font-semibold rounded-lg hover:shadow-md hover:scale-[1.02] transition-all"
              >
                <Cloud size={13} />
          ...
          >
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Automatically upload compressed
                files to Drive
              </p>
            </div>
          </label>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-slate-500 max-w-2xl mx-auto">
            Reduce file size while maintaining
            quality with intelligent compression
          </p>
        </div>
          ...
          {[
            {
              ...
              sub: "Process in seconds",
            },
            {
              icon: Shield,
              label: "Quality Control",
              sub: "Adjustable compression level",
            },
            {
              icon: CloudUpload,
              label: "Cloud Backup",
              sub: "Save to Google Drive",
            },
            {
              icon: Clock,
